# -*- coding: utf-8 -*-
"""
Created on Sun Apr  2 22:04:54 2017
...
cor_SalePrice=cor_train.corr()["SalePrice"]
print(cor_SalePrice)
for columns in cor_train.corr():
    if abs(cor_SalePrice[columns])>0.5:
        print(columns)
...
alls["SimplGarageQual"] = alls.GarageQual.replace({1 : 1, # bad
                                                     2 : 1, 3 : 1, # average
                                                     4 : 2, 5 : 2 # good
                                                    })

alls["SimplExterQual"] = alls.ExterQual.replace({1 : 1, # bad
                                                   2 : 1, 3 : 1, # average
                                                   4 : 2, 5 : 2 # good
                                                  })

    
alls["OverallQual-s2"] = alls["OverallQual"] ** 2
alls["OverallQual-s3"] = alls["OverallQual"] ** 3
alls["OverallQual-Sq"] = np.sqrt(alls["OverallQual"])

alls["GrLivArea-2"] = alls["GrLivArea"] ** 2
alls["GrLivArea-3"] = alls["GrLivArea"] ** 3
alls["GrLivArea-Sq"] = np.sqrt(alls["GrLivArea"])

alls["SimplOverallQual-s2"] = alls["SimplOverallQual"] ** 2
alls["SimplOverallQual-s3"] = alls["SimplOverallQual"] ** 3
alls["SimplOverallQual-Sq"] = np.sqrt(alls["SimplOverallQual"])

alls["ExterQual-2"] = alls["ExterQual"] ** 2
alls["ExterQual-3"] = alls["ExterQual"] ** 3
alls["ExterQual-Sq"] = np.sqrt(alls["ExterQual"])

alls["GarageCars-2"] = alls["GarageCars"] ** 2
alls["GarageCars-3"] = alls["GarageCars"] ** 3
alls["GarageCars-Sq"] = np.sqrt(alls["GarageCars"])

alls["SimplBsmtQual"] = alls.BsmtQual.replace({1 : 1, # bad
                                                 2 : 1, 3 : 1, # average
                                                 4 : 2, 5 : 2 # good
                                                })
alls["AllSF"] = alls["GrLivArea"] + alls["TotalBsmtSF"]

alls["SimplFireplaceQu"] = alls.FireplaceQu.replace({1 : 1, # bad
                                                       2 : 1, 3 : 1, # average
                                                       4 : 2, 5 : 2 # good
                                                      })
alls["TotalBath"] = alls["BsmtFullBath"] + (0.5 * alls["BsmtHalfBath"]) + alls["FullBath"] + (0.5 * alls["HalfBath"])

    
alls['1stFlr_2ndFlr_Sf'] = np.log1p(alls['1stFlrSF'] + alls['2ndFlrSF'])   



#    if abs(cor_SalePrice[columns])<0.03:
#        alls=alls.drop([columns], axis=1)
