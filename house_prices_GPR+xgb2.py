# -*- coding: utf-8 -*-
"""
Created on Mon Apr  3 00:35:29 2017
...
pre_all=np.append(prediction,prediction_train)

print(clf.steps[1][1].kernel_)


#prediction = np.expm1(prediction + y_log.mean())
#pre_all = np.expm1(pre_all + y_log.mean())
prediction = np.expm1(prediction + y_log.mean())
prediction_train = np.expm1(prediction_train + y_log.mean())    
    
    
def inverse_catcheck(df):
    iscat = np.zeros(df.shape[1])
    for c in range(df.shape[1]):
        if df.dtypes[c] != 'object':
            iscat[c] = 1
...
#train2 = train2.drop(inverse_catcheck(train2), axis = 1)
#test2 = test2.drop(inverse_catcheck(test2), axis = 1)
...
def factor_encoding(df):
    for column in df:
        if df[column].dtype == 'object':
            ...
            del df[column]
    return df
alls = alls.drop(inverse_catcheck(alls), axis = 1)


#alls['GPR']=0
#for i in range(len(pre_all)):
#    alls['GPR'][i]=pre_all[i]
    
train2 = alls[:train_shape]
train2 = factor_encoding(train2)
train2['GPR']=0
for i in range(len(prediction_train)):
    train2['GPR'][i]=prediction_train[i]


test2 = alls[train_shape:]
test2 = factor_encoding(test2)
test2['GPR']=0
for i in range(len(prediction)):
    test2['GPR'][i]=prediction[i]

#collist = set(NAcheck(train2) + NAcheck(test2))
#train2 = train2.drop(collist, axis = 1)
#test2 = test2.drop(collist, axis = 1)

for column in train2:
    if (column in test2.columns)==0:
        train2=train2.drop(column,axis=1)
        
for column in test2:
    if (column in train2.columns)==0:
        test2=test2.drop(column,axis=1)
...
dtest_array = xgb.DMatrix(pd.DataFrame.as_matrix(test2))
...
test2['SalePrice'] = list(ypred)
test2['Id']=0
  
for i in range(len(test2)):
    test2['Id'][i]=train_shape+i+1
#test['id']=Id
submission = test2[['Id','SalePrice']]
#submission = pd.concat([Id, list(ypred)], axis=1)
#submission = [Id test['SalePrice']]


submission.to_csv('submissionGX.csv',index=False)
